...
def splitData(dataset):
    ...
    for i in range(1,len(dataset)):
        Character.append([float(tk) for tk in dataset[i][0:-1]])
...
def max_min_norm_x(dataset):
    ...
    data_x3 = []
    for index in range(0, len(data_x), 6):
...
if __name__ == "__main__":
    ...
    #数据预处理
    Data = load_data_write(data_path)
    ...
    plt.plot(x, y_test[0:100])
    plt.plot(x, pre[0:100], color='red', linewidth=1, linestyle='--')
    plt.xlim((0, 100))
    plt.ylim((0, 1))
    plt.xlabel('测试样本个数')

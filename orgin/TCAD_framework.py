...
import numpy as np
import os
from kop_calculator import KOP_calculator, diff_forward
...
def model_pred_group(model, infile, outfile):
    os.system('run pred.py -data=%s -model=%s -output=%s' % (infile, model, outfile))
    #print(infile,model,outfile)
    pred.get_pred(infile,model,outfile)
    if not os.path.exists(outfile):
        raise FileNotFoundError(outfile)


# 1. train model
def get_model_and_time(train_file, outdir):
    train_file = os.path.abspath(train_file)
    model_file = os.path.join(outdir, 'model.h5')
    print(model_file)
    os.system('run train.py -data=%s -model=%s' % (train_file, model_file))
...
# 2. test model
def test_model(model, test_file, output_dir):
    ...
    for ln in contents:
        ...
        test_one_file(model, fn, pnr, output_dir)


def test_one_file(model, ftest, parameter, output_dir):
    ...
    with open(pred_file, 'w') as f:
        f.write('\n'.join(p_content))
    model_pred_group(model, pred_file, pred_rfile)
...
def calculate_error(config_file, test_file, output_dir):
    ...
    for i in range(len(file_names)):
        ...
        if np.sum(valid) <= 0:
            continue
        ids_m = np.log10(ids_m[valid])
        ids_s = np.loadtxt(pred_files[i], dtype='float', delimiter=',', skiprows=3)[:, 2][valid]
        
        lgg =  linecache.getline(pred_files[i],2).split(',')[0]
        rr =  linecache.getline(pred_files[i],2).split(',')[1]
        toxx = linecache.getline(pred_files[i],2).split(',')[2]
        
        ids_s = np.log10(np.abs(ids_s.reshape(-1)) + 1e-15)
        rmses.append(np.sqrt(np.mean(((ids_s - ids_m) / ids_m) ** 2)))
    ...
    lg = np.array(lg,dtype=float)
    r = np.array(r,dtype=float)
    tox = np.array(tox,dtype=float)
    '''plot'''
    print('lg=',lg,'r=',r,'tox=',tox)
    index = []
    index1 = []
    ...
    plot.para_test(vtgm_s_np[index],vtgm_m_np[index],lg[index])
    print('I=',vtgm_m_np[index],'Ipred=',vtgm_s_np[index],'lg=',lg[index],'r=',r[index],'tox=',tox[index])
   # print('I19=',idsat_m_np[index1],'lg75=',lg[index1],'r75=',r[index1],'tox75=',tox[index1])
...
def calculate_gummel_test(model, config_file, output_dir):
    ...
    for idx1, params in enumerate(p):
        ...
        for idx2, vg in enumerate(vg_all):
            ...
            with open(pred_file, 'w') as f:
                f.write('\n'.join(result))
            model_pred_group(model, pred_file, predr_file)
...
if __name__ == "__main__":
    ...
    # config_file = r'C:\Users\liangqm\Desktop\EDA2022_primarius\EDA2022\planar_data\config.txt'
    # train_file = r'C:\Users\liangqm\Desktop\EDA2022_primarius\EDA2022\GAA_data\parametertrain.txt'
    # test_file = r'C:\Users\liangqm\Desktop\EDA2022_primarius\EDA2022\GAA_data\parametertest.txt'
    # output_dir1 = 'result'
    ...
    #model = get_model_and_time(train_file, output_dir1)
    model = './outGAA100/model.h5'
    
   # time_for_train = time.time() - start_train
    #start_pred = time.time()
    test_model(model, test_file, output_dir1)
    ...
    calculate_gummel_test(model, config_file, output_dir1)
